...
contract Wolin is FlashLoanReceiverBase {
        ...
        returns (bool)
    {
        ...
        if (keccak256(abi.encodePacked((condition))) == keccak256(abi.encodePacked(("condition1")))) {
            ...
            require(IERC20(loanedToken).approve(address(kyberNetworkProxy), amounts[0]), "approval to srcQty failed");
            
            uint256 minConversionRate = getConversionRatesFromKyberNetwork(IERC20(loanedToken), IERC20(daiTokenAddress), amounts[0]);
            ...
            require(IERC20(daiTokenAddress).approve(address(uniswapRouter), minConversionRate), 'approve failed.');
            
            //uint[] memory amountOut = getPairRateFromUniswap(actualDestAmount * (10**6), getPath(daiTokenAddress, loanedToken));
            
            uniswapRouter.swapExactTokensForTokens(
                minConversionRate, //in
                0.1 ether, //outmin
    ...
    function myFlashLoanCall(address[] memory assets, uint256[] memory amounts, address daiAddress, uint256[] memory modes, bytes memory params) public {
